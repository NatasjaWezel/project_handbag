...
def main():
    ...
    vdw_distance_contact = radii.get_vdw_distance_contact(df, settings)

    coordinate_df = make_coordinate_df(df, settings, avg_fragment, radii)

    title = "Central fragment: " + settings.central_group_name + "\n" +\
            "Scattered contact atoms: " + settings.contact_group_name + "(" + settings.to_count_contact + ")"
