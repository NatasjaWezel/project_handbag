...
def make_coordinate_df(df, settings, avg_fragment, radii, again=False):
    try:
        if again:
            raise KeyError
...
def distances_closest_vdw_central(coordinate_df, avg_fragment, labels=""):
    ...
    # so we can use JIT for the distance calculation
...
def get_dihedral_and_h(CSV, central_group_name):
    ...
    methyl_model["angle"] = float(df.angle.item())
    methyl_model["distance"] = float(df.distance.item())
    methyl_model["first_degrees"] = int(df.first_deg.item())
...
def add_model_methyl(CSV, fragment, settings, radii):
    ...
    alpha = np.radians(methyl_model['angle'])
    ...
    if methyl_model["first_degrees"] % 120 == 0:
        dihedrals = [0, 120, 240]
    else:
        dihedrals = [60, 180, 300]
